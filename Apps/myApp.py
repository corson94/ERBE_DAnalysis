...
from Model.Cleaning.AllHouseholds import AllHouseholds
...
@app.callback(
    ...
    # Output('household_id_label', 'children'),
    # Output('household_id', 'min'),
    # Output('household_id', 'max'),
...
def update_figure(group_type: 'str', allHhousehold_plot_type, click_info, household_plot_type, household_id):
    ...
    fig_allHh = getattr(AllHouseholds(group_type), allHhousehold_plot_type)(write=None)
    starting_id = Household.range_of_Households[Household.groups.index(group_type)][0]
    # maximum_id = Household.range_of_Households[Household.groups.index(group_type)][1]
    # string = f"Household ID ({minimum_id} - {maximum_id}): "

    if triggered_id is None:
        fig_hh = getattr(Household(int(starting_id)), household_plot_type)(write=None)
        id_value = starting_id

    elif triggered_id == 'allHouseholds_plot':
        click_hhid = click_info['points'][0]['customdata'][0]
        fig_hh = getattr(Household(int(click_hhid)), household_plot_type)(write=None)
        id_value = click_hhid

    else:
        fig_hh = getattr(Household(int(household_id)), household_plot_type)(write=None)
        id_value = household_id

    return fig_allHh, fig_hh, id_value  # string, minimum_id, maximum_id,
